...
task("multiPriceFeedTransmit", "Submits an answer to multiPriceFeed")
  ...
  .setAction(async (_taskArgs, hre) => {
    ...
    let received_pairs = _taskArgs.pricepairs.split(" ");
    let received_answers = _taskArgs.answers.split(" ");
    ...
    let tx = await contract.connect(validator).transmit(pairs, received_answers);
