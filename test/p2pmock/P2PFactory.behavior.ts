...
export function shouldBehaveLikeFluxP2PFactory(): void {
  ...
  it("should let admin add providers", async function () {
    ...
    await this.factory.connect(this.signers.admin).addProvider(this.provider3tobe.address);
  ...
  it("doesn't let nonadmin add providers", async function () {
    ...
    await expect(
      this.factory.connect(this.signers.nonadmin).addProvider(this.provider3tobe.address),
    ).to.be.revertedWith("NOT AN ADMIN, CANT ADD PROVIDER");
  });
  ...
  it("should let admin remove providers", async function () {
    ...
    let sigs = [p1_sig, p2_sig];
    await this.factory.connect(this.signers.admin).revokeProvider(this.provider2.address);
  ...
  it("doesn't let nonadmin remove providers", async function () {
    ...
    await expect(
      this.factory.connect(this.signers.nonadmin).revokeProvider(this.provider3tobe.address),
    ).to.be.revertedWith("NOT AN ADMIN, CANT REVOKE PROVIDER");
  });
